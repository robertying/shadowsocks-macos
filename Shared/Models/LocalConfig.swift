...
final class LocalConfig: Hashable, Codable, Identifiable, ObservableObject {
  ...
  init(from decoder: Decoder) throws {
    ...
    proto = (try? container.decode(Protocol.self, forKey: .proto)) ?? Protocol.socks
    localAddress = (try? container.decode(String.self, forKey: .localAddress)) ?? "127.0.0.1"
    localPort = (try? container.decode(UInt16.self, forKey: .localPort)) ?? 1080
  }
