...
struct Config: Codable {
    ...
    init() {
        self.localConfigs = [
            LocalConfig(
                proto: .socks, localAddress: "127.0.0.1", localPort: 1080),
            LocalConfig(
    ...
    init(from decoder: Decoder) throws {
        ...
        localConfigs =
            (try? container.decode([LocalConfig].self, forKey: .localConfigs))
            ?? []
        serverConfigs =
            (try? container.decode([ServerConfig].self, forKey: .serverConfigs))
            ?? []
        mode =
            (try? container.decode(Mode.self, forKey: .mode)) ?? Mode.tcp_only
        noDelay = (try? container.decode(Bool.self, forKey: .noDelay)) ?? true
        nofile = (try? container.decode(UInt.self, forKey: .nofile)) ?? 65535
        ipv6First =
            (try? container.decode(Bool.self, forKey: .ipv6First)) ?? false
    }
