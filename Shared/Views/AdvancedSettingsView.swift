...
struct AdvancedSettingsView: View {
    ...
    var socks5Config: Binding<LocalConfig> {
        set(newConfig) {
            if let index = configData.config.localConfigs.firstIndex(where: {
                $0.proto == .socks
            }) {
                ...
                configData.config.localConfigs.append(
                    LocalConfig(
                        proto: .socks, localAddress: "127.0.0.1",
                        localPort: 1080))
            }
        }
        get {
            if let index = configData.config.localConfigs.firstIndex(where: {
                ...
                configData.config.localConfigs.append(
                    LocalConfig(
                        proto: .socks, localAddress: "127.0.0.1",
                        localPort: 1080))
                return $configData.config.localConfigs[
                    configData.config.localConfigs.count - 1]
            }
        }
    }
    ...
    var httpConfig: Binding<LocalConfig> {
        set(newConfig) {
            if let index = configData.config.localConfigs.firstIndex(where: {
                ...
                configData.config.localConfigs.append(
                    LocalConfig(
                        proto: .http, localAddress: "127.0.0.1", localPort: 1090
                    ))
            }
        }
        get {
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                VStack(alignment: .leading) {
                    ...
                    HStack {
                        ...
                        Text(":")
                        TextField(
                            "Bind Port", value: socks5Config.localPort,
                ...
                VStack(alignment: .leading) {
                    ...
                    HStack {
                        ...
                        Text(":")
                        TextField(
                            "Bind Port", value: httpConfig.localPort,
                            formatter: NumberFormatter()
                        )
                        .frame(width: portTextFieldWidth)
                    }
                ...
                HStack {
                    Text("nofile")
                    TextField(
                        "", value: $configData.config.nofile,
                        formatter: NumberFormatter()
                    ).frame(
                        width: portTextFieldWidth)
                }
            }.padding().frame(width: 300).position(
                x: gp.size.width / 2, y: gp.size.height / 2)
