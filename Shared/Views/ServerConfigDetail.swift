        ...
        GeometryReader { gp in
            VStack {
                HStack {
                    ...
                    TextField("", text: $config.remark ?? "")
                        .disableAutocorrection(true)
                }
                HStack {
                    Label(text: "Server *", minWidth: labelWidth)
                    TextField("IP / domain", text: $config.address)
                        .disableAutocorrection(true)
                }
                HStack {
                    Label(text: "Port *", minWidth: labelWidth)
                    TextField(
                        "1 - 65535", value: $config.port,
                ...
                HStack {
                    ...
                    TextField("", text: $config.password).disableAutocorrection(
                        true)
                }
                Picker(
                    selection: $config.plugin,
                    label: Label(text: "Plugin", minWidth: labelWidth)
                ) {
                ...
                HStack {
                    ...
                    TextField("", text: $config.pluginOpts ?? "")
                        .disableAutocorrection(true)
                }
            }.textFieldStyle(RoundedBorderTextFieldStyle()).padding().frame(
