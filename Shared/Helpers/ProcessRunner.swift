...
final class ProcessRunner {
    ...
    private static func kill(_ name: String) {
        ...
        terminateObserver = NotificationCenter.default.addObserver(
            forName: Process.didTerminateNotification, object: killTask,
            queue: nil
        ) { notification in
    ...
    private static func unsetProxy() {
        ...
        proxyTask.launchPath =
            DirectoryHelper.binRootFolder.appendingPathComponent(
                "networksetup.sh"
            ).path
        ...
        var terminateObserver: NSObjectProtocol!
        terminateObserver = NotificationCenter.default.addObserver(
            forName: Process.didTerminateNotification, object: proxyTask,
            queue: nil
        ) { notification in
    ...
    private static func setProxy() {
        let proxyType =
            UserDefaults.standard.string(forKey: "proxyType")
            ?? ProxyType.bypass_china_ips.rawValue
        ...
        configData.load(
            onLoad: {
                ...
                let httpConfig = configData.config.localConfigs.filter {
                    $0.proto == .http
                }[0]
                proxyTask.arguments = [
                    "set", httpConfig.localAddress,
                    String(httpConfig.localPort),
                ]

                var terminateObserver: NSObjectProtocol!
                terminateObserver = NotificationCenter.default.addObserver(
                    forName: Process.didTerminateNotification,
                    object: proxyTask, queue: nil
                ) { notification in
    ...
    static func start() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
            ...
            if proxyType == ProxyType.manual.rawValue {
                ...
                let aclUrl = DirectoryHelper.supportFolder
                    .appendingPathComponent(proxyType)
                    .appendingPathExtension("acl")
                task.arguments = [
                    "-c", ConfigData.fileURL.path, "--acl", aclUrl.path,
                ]
            }
            ...
            outputObserver = NotificationCenter.default.addObserver(
                forName: .NSFileHandleDataAvailable, object: outputHandle,
                queue: nil
            ) { notification in
            ...
            errorObserver = NotificationCenter.default.addObserver(
                forName: .NSFileHandleDataAvailable, object: errorHandle,
                queue: nil
            ) { notification in
            ...
            var terminateObserver: NSObjectProtocol!
            terminateObserver = NotificationCenter.default.addObserver(
                forName: Process.didTerminateNotification, object: task,
                queue: nil
            ) { notification in
                running = false
                loading = false
