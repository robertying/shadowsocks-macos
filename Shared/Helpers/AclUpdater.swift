...
final class AclUpdater {
    ...
    private static let aclFiles = [
        "chinadomainlist.acl", "chinaiplist.acl", "chinalist.acl",
        "gfwlist.acl", "lanlist.acl",
    ]
    ...
    static func copyAclFilesFromBundleToSupportFolder() {
        ...
        do {
            let dirContents = try fileManager.contentsOfDirectory(
                atPath: DirectoryHelper.aclFolder.path)
            for fileName in dirContents {
                let sourceURL = DirectoryHelper.aclFolder
                    .appendingPathComponent(fileName)
                let destURL = DirectoryHelper.supportFolder
                    .appendingPathComponent(fileName)
    ...
    private static func downloadAclFilesToSupportFolder(_ url: URL) {
        let downloadTask = URLSession.shared.downloadTask(with: url) {
            ...
            do {
                let savedURL = DirectoryHelper.supportFolder
